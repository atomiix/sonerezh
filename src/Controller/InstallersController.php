...
class InstallersController extends AppController
{
    ...
    public function index()
    {
        ...
        if (empty($pdo_drivers)) {
            ...
            foreach ($drivers as $driver) {
                if (in_array($driver, $pdo_drivers)) {
                    $requirements[$driver] = array('label' => 'success', 'message' => __('The {0} driver is installed.', $driver));
                    ...
                    $requirements[$driver] = array('label' => 'warning', 'message' => __('The {0} driver is required if you want to use Sonerezh with {1}', $driver, $driver));
                }
            }
        }
        ...
        if ($is_config_writable) {
            $requirements['conf'] = array('label' => 'success', 'message' => __('{0} is writable', CONFIG));
        } else {
            $requirements['conf'] = array('label' => 'danger', 'message' => __('{0} is not writable', CONFIG));
            $missing_requirements = true;
        }
        ...
        if ($this->request->is('post')) {
            ...
            $datasources = array('mysql', 'postgresql', 'sqlite');

            if (in_array($this->request->getData('DB.datasource'), $datasources)) {
			...
			return $this->redirect(array('controller' => 'songs', 'action' => 'import'));
        }
    }
