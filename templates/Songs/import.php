...
<script type="text/javascript">
    ...
    function ajaxImport() {
        ...
        xhr.open("POST", "<?php echo $this->Url->build(array('controller' => 'songs', 'action' => 'import')); ?>", true);
        ...
        <div class="panel panel-primary" id="import-panel">
            <div class="panel-heading" id="import-panel-header">
                <?php if ($to_import_count > 0): ?>
                    <?php echo __n("{0} song detected", "{0} songs detected", $to_import_count, $to_import_count) . ' ' . __('({0} already imported)', $already_imported_count) . '<br/>'; ?>
                <?php endif; ?>
                <?php if ($to_update_count > 0): ?>
                    <?php echo __n("{0} song updated on the file system", "{0} songs updated on the file system", $to_update_count, $to_update_count) . '<br/>'; ?>
                <?php endif; ?>
                <?php if ($to_remove_count > 0): ?>
                    <?php echo __n("{0} song removed from file system", "{0} songs removed from file system", $to_remove_count, $to_remove_count) . '<br/>'; ?>
                <?php endif; ?>
            </div>
