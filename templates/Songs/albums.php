...
<div class="col-lg-12" data-scroll-container="true" data-view="albums">
    <?php if (!empty($latests) && $this->Paginator->current('Song') == 1): ?>
        ...
        <?php foreach ($latests as $album): ?>
            ...
            <div class="col-md-2 col-sm-3 col-xs-4 action-expend <?php echo $hidden; ?>" data-band="<?php echo h($album->band); ?>" data-album="<?php echo h($album->album); ?>" data-scroll-content="true">
                <?php echo $this->Image->lazyload($this->Image->resizedPath($album->cover, 220, 220), array('class' => 'img-responsive cover lzld', 'style' => 'cursor: pointer;')); ?>
                <h4 class="truncated-name" title="<?php echo h($album->album); ?>">
                    <?php echo h($album->album); ?>
                    <small>
                        <br /><?php echo h($album->band); ?>
                    </small>
                </h4>
            </div>
            <?php
	...
	<?php foreach ($songs as $album): ?>
        <div class="col-md-2 col-sm-3 col-xs-4 action-expend" data-band="<?php echo h($album->band); ?>" data-album="<?php echo h($album->album); ?>" data-scroll-content="true">
            <?php echo $this->Image->lazyload($this->Image->resizedPath($album->cover, 220, 220), array('class' => 'img-responsive cover lzld', 'style' => 'cursor: pointer;')); ?>
            <h4 class="truncated-name" title="<?php echo h($album->album); ?>">
                <?php echo h($album->album); ?>
                <small>
