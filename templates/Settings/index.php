...
$templates = [
	'inputContainer' => '<div class="{{type}}{{divClass}}{{required}}"{{divId}}>{{content}}</div>',
	'inputContainerError' => '<div class="{{type}}{{divClass}}{{required}}"{{divId}}>{{content}}{{error}}</div>',
	'error' => '<div class="text-danger" id="{{id}}">{{content}}</div>'
];
...
<div class="row" style="margin-bottom: 50px;">
    <div class="col-md-8 col-xs-12">
        <div class="col-xs-12">
            ...
            <hr />
            <?php
            echo $this->Form->create($settings, ['id' => 'SettingIndexForm']);
            $this->Form->unlockField('quality');
            $this->Form->unlockField('rootpaths');
            ...
            <div class="rootpath">
            <?php
            if (count($settings->rootpaths) > 0) echo $this->Form->control("rootpaths.0.id");
            echo $this->Form->error('rootpaths.0.rootpath');
            echo $this->Form->control('rootpaths.0.rootpath', array(
                'type'  => 'text',
                'label' => false,
				'divId' => 'root-path-input-group',
                'divClass' => 'input-group',
				'templates' => ['error' => '<div class="text-danger" id="{{id}}">{{content}}</div>'],
                'error' => false,
            ...
            <?php
            foreach ($settings->rootpaths as $i => &$rootpath) {
                if (!$i) continue;
                echo '<div class="rootpath">';
                if (isset($rootpath->id)) echo $this->Form->control("rootpaths.$i.id");
                echo $this->Form->error("rootpaths.$i.rootpath");
                echo $this->Form->control("rootpaths.$i.rootpath", array(
                    'type'  => 'text',
                    'label' => false,
                    'divClass'   => 'input-group',
                    'error' => false,
            ...
            echo $this->Form->control('enable_mail_notification', array(
                'type'  => 'checkbox',
                'label' => __('Enable mail notifications.'),
                'disabled' => !file_exists(CONFIG.'email.php')
            ));
            ?>
            ...
            <small>
                <span class="help-block">
                    ...
                    <?php echo $this->Html->link(
                        ...
                        'https://www.sonerezh.bzh/user-guides/settings.html#email-notifications',
                    ...
                    <?php else: ?>
                        ...
                        <div class="col-xs-12">
                            <div class="col-xs-6 separator-right text-right">
                                ...
                                <?php echo $this->Form->control('from_mp3', array('type' => 'checkbox', 'label' => 'MPEG-1/2 Audio Layer 3 (MP3)')); ?>
                                <?php echo $this->Form->control('from_ogg', array('type' => 'checkbox', 'label' => 'Ogg Vorbis (OGG)')); ?>
                                <?php echo $this->Form->control('from_flac', array('type' => 'checkbox', 'label' => 'Free Lossless Audio Codec (FLAC)')); ?>
                                <?php echo $this->Form->control('from_aac', array('type' => 'checkbox', 'label' => 'Advanced Audio Coding (AAC)', 'disabled' => 'disabled', 'checked' => 'checked')); ?>
                            </div>
                            <div class="col-xs-6">
                                <h5><strong><?php echo __('Destination format'); ?></strong></h5>
                                <?php echo $this->Form->control('convert_to', array('label' => false, 'type' => 'radio', 'options' => array('mp3' => 'MP3', 'ogg' => 'OGG'), 'templates' => ['error' => '<p class="text-danger" id="{{id}}">{{content}}</p>'])); ?>
                                <h5><strong><?php echo __('Quality'); ?></strong></h5>
                                <?php echo $this->Form->control('quality', array('type' => 'hidden')); ?>
                                <div class="row quality">
                                    <div class="col-xs-9"><div id="quality-slider"></div></div>
                                    <div class="col-xs-3"><?php echo $settings->quality; ?></div>
                                </div>
                            </div>
                        </div>
                    <?php endif; ?>
			...
			<?php echo $this->Form->submit(__('Submit'), array('class' => 'btn btn-success pull-right')); ?>
        ...
        <div class="col-xs-12">
            ...
            <div class="col-sm-4 col-xs-12">
                <?php echo $this->Form->postLink(__('Reset the database'), array('action' => 'truncate'), array('class' => 'btn btn-danger clearfix center-block', 'confirm' => __('Are you sure? All your songs and playlists will disappear!'))); ?>
            </div>
        </div>
    </div>
