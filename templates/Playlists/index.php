        ...
        <ul class="list-group">
            ...
            <li class="list-group-item ">
                ...
                <?php echo $this->Form->postLink(
                    ...
                    array('class' => 'btn btn-xs btn-danger btn-mgmt btn-playlist-delete', 'escape' => false, 'confirm' => __('Are you sure?')),
                ); ?>
            </li>
            <?php endforeach; ?>
        </ul>
                    ...
                    <?php foreach ($playlist as $song): ?>
                        <tr data-id="<?php echo h($song->song->id); ?>" data-scroll-content="true">
                            <td class="track-number">
                                <span class="song-number"><?php echo h($song->sort); ?></span>
                            </td>
                            ...
                            <td class="text-right playtime-cell">
                                ...
                                <span class="glyphicon glyphicon-play song-controls action-play" data-action="play" title="<?php echo __('Play'); ?>"></span><span class="glyphicon glyphicon-pause song-controls action-pause" data-action="pause" title="<?php echo __('Pause'); ?>"></span><?php
								echo $this->Form->postLink(
                                        '<span class="glyphicon glyphicon-remove song-controls action-remove-from-playlist" title="'.__("Remove").'"></span>',
                                        array('controller' => 'playlist_memberships', 'action' => 'remove', $song->id),
                                        array('style' => 'text-decoration: none;', 'escape' => false, 'confirm' => __('Remove').' '.h($song->song->title).' '.__('from the playlist?'))
                                ); ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<div id="add-playlist-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addPlaylistModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <?php echo $this->Form->create(new \App\Model\Entity\Playlist(), array('url' => array('controller' => 'playlists', 'action' => 'add'))); ?>
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><?php echo __('Create a playlist'); ?></h4>
            </div>
            <div class="modal-body">
                <?php echo $this->Form->control('title', array('placeholder' => __('Title'))); ?>
            </div>
...
<?php if (!empty($playlists)): ?>
    <?php foreach ($playlists as $id => $name): ?>
        <div id="<?php echo 'edit-playlist-'.$id.'-modal'; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="editPlaylistModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <?php echo $this->Form->create(new \App\Model\Entity\Playlist(), array('url' => array('controller' => 'playlists', 'action' => 'edit', $id))); ?>
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title"><?php echo __('Rename a playlist'); ?></h4>
                    </div>
                    <div class="modal-body">
                        <?php echo $this->Form->control('title', array('placeholder' => __('New title'), 'value' => $name)); ?>
                    </div>
                    <div class="modal-footer">
